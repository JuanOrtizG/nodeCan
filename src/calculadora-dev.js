...
const socket = io()
  ...
  let input_formula   = document.getElementById('formula')
  ...
  socket.on('lectura',(data)=>{
    ...
    chart.data.labels.push(counter);
        chart.data.datasets.forEach(dataset => {
          ...
          let A = parseInt(data.A,16)
          let B = parseInt(data.B,16)
          let C = parseInt(data.C,16)
          let D = parseInt(data.D,16)
          calculo = eval(input_formula.value)
          console.log(calculo)
          //conversionDecimal = parseInt(B, 16);
          //dataset.data.push(conversionDecimal);
          dataset.data.push(calculo)
        });
        counter++;
        chart.update();
